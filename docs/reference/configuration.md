...
| `EVYDENCE_WORKER_OWNED_PARSER_SIDE_EFFECTS` | No | `false` | Optional hardening mode for CycloneDX SBOM uploads. When set to `true`, the API stores an accepted SBOM record and the outbox worker populates parser-derived SBOM fields from the tenant-prefixed raw payload after digest verification. |
