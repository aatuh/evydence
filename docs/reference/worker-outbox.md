...
OpenAPI contract, and DSSE build-attestation normalized fields when
`EVYDENCE_WORKER_OWNED_PARSER_SIDE_EFFECTS=true`. OpenVEX upload paths still
create initial normalized records before worker replay validates and backfills
missing fields. Moving that remaining parser side effect fully out of the
