...
The archive verifier accepts only the customer package files listed in this
reference. It rejects path-traversal entries, nested paths, absolute paths,
backslash-separated paths, empty names, duplicate entries, unexpected files,
duplicate JSON keys, and undeclared `vulnerability-decisions.json` entries
before trusting `manifest.json`, `package.json`, `verification.json`, or the
declared decision export.
