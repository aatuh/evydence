...
| CI provider provenance | operator-owned/provider-owned | GitHub Actions build metadata, offline DSSE PAE/in-toto Statement v1/SLSA provenance v1 verification against tenant roots and policy, collector keys, and CI upload examples. | Branch protection, required workflow policy, runner trust, OIDC token policy where used, protected environments, and provider audit logs. |
