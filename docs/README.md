...
- [Evidence format compatibility](reference/evidence-format-compatibility.md): tested CycloneDX, SPDX, VEX, DSSE/in-toto, and scanner input shapes, parser versions, limits, and unsupported cases.
- [Verification results](reference/verification-results.md): machine result states, assurance-profile fields, legacy migration behavior, and customer-package limits.
