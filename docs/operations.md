...
- Human SSO session records are admin-managed, and authenticated SSO sessions can revoke themselves through API-first logout. Current endpoints can verify supplied OIDC tokens and SAML assertions against tenant-configured local trust material, but they do not implement production browser login callbacks, live provider discovery/API validation, group synchronization, or browser cookie/session handling.
