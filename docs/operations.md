...
- Customer portal access tokens are returned once by `POST /v1/customer-portal/access`, expire, and are stored as hashes. Successful manifest exchanges and ZIP downloads write audit-chain entries under both the portal access record and package record; failed known-prefix attempts are recorded as unverified access events. Supplied portal tokens are never stored in those entries. Deployments should still use reverse-proxy or API-gateway throttling for the unauthenticated portal endpoint.
