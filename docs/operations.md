...
- Human SSO session records are admin-managed, authenticated SSO sessions can revoke themselves through API-first logout, and SSO credential exchange can set an HttpOnly session cookie after locally verifying an OIDC ID token or SAML assertion against tenant-configured trust material. Current endpoints can refresh OIDC public JWKS from discovery metadata, but they do not implement live provider API validation or group synchronization.
