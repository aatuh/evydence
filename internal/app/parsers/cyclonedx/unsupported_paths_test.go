...
func TestParseBoundedReportsEveryPreservedStandardConstructItDoesNotNormalize(t *testing.T) {
	raw := []byte(`{"bomFormat":"CycloneDX","specVersion":"1.6","serialNumber":"urn:uuid:3e671687-395b-41f5-a30f-a58921a69b79","version":1,"properties":[{"name":"root","value":"kept"}],"components":[{"type":"library","name":"parent","mime-type":"application/java-archive","group":"example.org","cpe":"cpe:2.3:a:example:parent:1.0:*:*:*:*:*:*:*","scope":"required","modified":false,"components":[{"type":"library","name":"child"}],"tags":["runtime"]}],"dependencies":[{"ref":"parent","provides":["child"]}]}`)
	got, err := ParseBounded(raw, DefaultLimits(1<<20))
	if err != nil {
		t.Fatal(err)
	}
	for _, want := range []string{"serialNumber", "version", "properties", "components[].mime-type", "components[].group", "components[].cpe", "components[].scope", "components[].modified", "components[].components", "components[].tags", "dependencies[].provides"} {
...
func TestParserVersionTracksNormalizationSemanticChange(t *testing.T) {
	if ParserVersion != "cyclonedx-json.v1.3.4" {
