...
var verificationProfileDefinitions = []VerificationProfileDefinition{
	...
	{ID: VerificationProfileDSSEAttestationSignature, Version: VerificationProfileSchemaVersion, RequiredChecks: []string{"dsse_pae_signature", "trusted_root", "payload_type", "predicate_type", "subject_digest", "builder_identity", "policy_required_claims"}, OptionalChecks: []string{"payload_digest", "trusted_root_version", "verifier_library"}, Canonicalization: "DSSE PAE over the exact payload type and decoded payload bytes; the signed payload is parsed as an in-toto Statement v1", HashAlgorithm: "SHA-256 artifact subject digest and stored envelope payload hash", SignatureAlgorithm: "Ed25519 DSSE PAE signature", TrustRootPolicy: "an active tenant DSSE root with immutable explicit predicate, builder, and required-claims policy", IdentityPolicy: "verified signature key, builder identity, and policy-required claims must all match the same configured tenant root", IssuerPolicy: "not applicable to the supported explicit public-key profile", TransparencyPolicy: "not evaluated", ClockPolicy: "not evaluated", RevocationPolicy: "only active versioned roots are accepted; historical validity is not established", OfflinePolicy: "stored original envelope, explicitly configured tenant root, and registered release-artifact subjects are required; the profile makes no network request"},
