...
var verificationProfileDefinitions = []VerificationProfileDefinition{
	...
	{ID: VerificationProfileCosignFull, Version: VerificationProfileSchemaVersion, RequiredChecks: []string{"digest_binding_assessed", "signature_material_present", "cryptographic_signature_verified", "certificate_identity_policy", "transparency_inclusion_proof"}, OptionalChecks: []string{"rekor_metadata_present", "certificate_chain", "certificate_validity", "certificate_issuer_policy", "certificate_revocation"}, Canonicalization: "OCI artifact digest and exact Cosign signed material", HashAlgorithm: "SHA-256 OCI subject digest", SignatureAlgorithm: "algorithm encoded by the verified Cosign bundle", TrustRootPolicy: "operator or tenant configured Fulcio and public-key roots with recorded version", IdentityPolicy: "caller-supplied expected identity is matched against verified certificate identity", IssuerPolicy: "caller-supplied expected issuer is matched against verified certificate issuer", TransparencyPolicy: "required profiles verify Rekor inclusion proof and checkpoint; no network downgrade is allowed", ClockPolicy: "certificate and checkpoint times are evaluated against the profile evaluation time", RevocationPolicy: "verified revocation evidence is required when the profile declares it", OfflinePolicy: "only a complete offline bundle and configured roots may satisfy an offline profile"},
