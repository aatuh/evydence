...
broad `GITHUB_TOKEN` write permissions: `EVYDENCE_RELEASE_PUBLISH_TOKEN` for the
separate draft-release publication job and `EVYDENCE_GHCR_PUBLISH_TOKEN` for
container image publication. The container signing job is the only
container-image job with `id-token: write` for keyless cosign signing. Those
secrets are external repository settings and are not release evidence artifacts.
